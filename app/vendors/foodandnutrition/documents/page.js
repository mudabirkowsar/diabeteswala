...
import FoodAPI from '../../../services/FoodVendorAPI'; // Service file mapped to FoodVendorAPI.js

export default function FoodDocumentsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!textData.documentState.trim() || !textData.issuingAuthority.trim() || !textData.fssaiNumber.trim()) {
      return triggerNotification("Please fill in State, Issuing Authority, and FSSAI License Number.", "warning");
    }
    ...
    try {
      ...
      Object.keys(textData).forEach((key) => {
        if (textData[key].trim()) {
          payload.append(key, textData[key].trim());
        }
      });
      ...
      const response = await FoodAPI.uploadFoodDocuments(payload);

      if (response && response.success) {
        setShowSuccessModal(true);
      } else {
        triggerNotification(response.message || "Failed to submit kitchen credentials.", "error");
      }
    } catch (err) {
      console.error(err);
      triggerNotification(
        err.response?.data?.message || "An error occurred while uploading operational records.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] py-8 px-4 sm:px-6 lg:px-12 antialiased select-none">
      ...
      <div className="max-w-4xl mx-auto mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            <Flame className="text-[#3d3f96]" /> Kitchen Credentials
          </h1>
          <p className="text-slate-500 font-semibold text-xs mt-1">Submit your FSSAI credentials, kitchen photos, and municipality operating certifications.</p>
        </div>
        <button 
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        {/* Left Side: Parameters, descriptions, and metadata */}
        <div className="md:col-span-5 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
            <h2 className="text-sm font-black text-slate-800 border-b border-slate-50 pb-2">Facility Parameters</h2>

            {/* About text input */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">About Your Service</label>
              <textarea
                name="about"
                rows={3}
                value={textData.about}
                onChange={handleTextChange}
                placeholder="Describe your kitchen facility, menu, meal preparation models, or specialities..."
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={handleTextChange}
                placeholder="e.g. FSSAI Authority of India"
                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-700 outline-none focus:border-[#3d3f96] transition-all"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">FSSAI License Number <span className="text-red-500">*</span></label>
              <input
                required
                type="text"
                name="fssaiNumber"
                value={textData.fssaiNumber}
                onChange={handleTextChange}
            ...
            {/* GST Number */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">GST Registration Number (Optional)</label>
              <input
                type="text"
                name="gstNumber"
        ...
        <div className="md:col-span-7 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            <h2 className="text-sm font-black text-slate-800 border-b border-slate-50 pb-2">Required Certificates</h2>

            {/* Single File Upload: Profile Image */}
            <div className="space-y-2">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Kitchen Display / Profile Image</p>
              {!files.profileImage ? (
                <label className="border-2 border-dashed border-slate-200 hover:border-[#3d3f96] rounded-2xl p-4 flex flex-col items-center justify-center gap-1 cursor-pointer transition-all bg-slate-50/50">
                  ...
                  <span className="text-xs font-bold text-slate-600">Choose Kitchen Photo</span>
                  <input
                    type="file"
                    accept="image/*"
            ...
            {[
              { label: 'FSSAI Registration Certificates', key: 'fssaiCertificates' },
              { label: 'Kitchen Premises & Gallery Images', key: 'kitchenImages' },
              { label: 'GST Certificate Forms (If Available)', key: 'gstCertificates' },
              { label: 'Other Licenses & Municipality Approvals', key: 'otherCertificates' }
            ].map((field) => (
              <div key={field.key} className="space-y-2">
                ...
                <label className="border-2 border-dashed border-slate-200 hover:border-[#3d3f96] rounded-2xl p-4 flex flex-col items-center justify-center gap-1 cursor-pointer transition-all bg-slate-50/50">
                  ...
                  <span className="text-xs font-bold text-slate-600">Attach Papers</span>
                  <input
                    multiple
                    type="file"
            ...
            >
              {loading ? (
                ...
                <>
                  <span>Submit Operational File</span>
                  <Upload size={18} />
                </>
              )}
            </button>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] max-w-md w-full p-8 shadow-2xl relative z-10 border border-slate-100 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Onboarding Logs Received</h3>
            <p className="text-xs text-slate-500 font-semibold mt-3 leading-relaxed">
              Your kitchen operating licenses and gallery files have been submitted successfully. Your profile verification state is now <strong className="text-[#3d3f96]">Pending Admin Approval</strong>.
            </p>
            ...
            <div className="mt-4 p-4 bg-amber-50/60 rounded-2xl border border-amber-100/50 flex items-start gap-2.5 text-left">
              ...
              <div>
                <p className="text-[10px] font-black text-amber-800 uppercase tracking-wider">Review in Progress</p>
                <p className="text-[11px] text-amber-700 font-medium mt-0.5">Please wait for our vetting team to check your FSSAI registration papers and workspace parameters. This process normally spans 24-48 hours.</p>
              </div>
            </div>
