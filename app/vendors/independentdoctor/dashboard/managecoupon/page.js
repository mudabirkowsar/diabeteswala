...
// Import Doctor API service functions
import IndependentDoctorAPI from '../../../../services/IndependentDoctorAPI'; // Adjust relative path based on your folder structure

export default function DoctorCouponsPage() {
  // --- Data States ---
  ...
  // --- 1. Fetch All Doctor & Global Admin Coupons ---
  const fetchCoupons = async () => {
    setLoading(true);
    try {
      const response = await IndependentDoctorAPI.getDoctorCouponsList();
      if (response && response.success) {
        setCoupons(response.data || []);
      } else {
        toast.error("Failed to load doctor coupons.");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleToggleStatus = async (coupon) => {
    ...
    try {
      const response = await IndependentDoctorAPI.toggleDoctorCouponStatus(coupon._id);
      if (response && response.success) {
        toast.success(response.message || `Coupon is now ${!coupon.isActive ? 'Live' : 'Paused'}`);
  ...
  const handleDeleteCoupon = async (coupon) => {
    if (coupon.isAdminCreated) {
      toast.error("Admin global coupons cannot be deleted by doctor.");
      return;
    }
    ...
    try {
      const response = await IndependentDoctorAPI.deleteDoctorCoupon(coupon._id);
      if (response && response.success) {
  ...
  const openEditModal = (coupon) => {
    if (coupon.isAdminCreated) {
      toast.error("Admin global coupons cannot be edited by doctor.");
      return;
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (modalMode === 'create') {
        response = await IndependentDoctorAPI.createDoctorCoupon(payload);
      } else {
        response = await IndependentDoctorAPI.updateDoctorCoupon(editingId, payload);
      }
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-10 py-6 pb-12 antialiased select-none text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Doctor Coupons &amp; Promotions</h1>
            <p className="text-xs text-slate-500 font-bold mt-1">Configure and manage discount vouchers for patient online consultations, clinic visits, and home visits.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          ...
          >
            <Plus size={15} /> CREATE DOCTOR COUPON
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-28 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Loading doctor vouchers catalog...</p>
        </div>
      ) : coupons.length > 0 ? (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <span className={`text-[9px] font-black border px-2.5 py-0.5 rounded-lg uppercase tracking-wider ${
                        ...
                        {coupon.isAdminCreated ? "ADMIN CAMPAIGN" : (coupon.vendorType || "DOCTOR")}
                      </span>
                    </div>
        ...
        <div className="flex flex-col items-center justify-center p-24 text-center bg-white rounded-3xl border border-slate-100 shadow-sm border-dashed">
          ...
          <p className="font-bold text-slate-700">No Active Promotional Vouchers</p>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 max-w-xl w-full p-6 sm:p-8 shadow-2xl relative flex flex-col [&::-webkit-scrollbar]:hidden text-left">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-900 text-base sm:text-lg uppercase tracking-tight">
                    {modalMode === 'create' ? 'Create Doctor Voucher' : 'Update Doctor Voucher'}
                  </h3>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Min Fee Subtotal (₹)</label>
                  <input
                    type="number"
                    value={formMinOrder}
