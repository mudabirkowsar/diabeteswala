...
import PharmacyAPI from '../../../services/PharmacyVendorAPI'; // Service file mapped to PharmacyVendorAPI.js
...
export default function PharmacyDocumentsPage() {
  ...
  const [textData, setTextData] = useState({
    ...
    about: '',
    isHomeDeliveryAvailable: 'true', // sent as string 'true'/'false' for multipart/form-data
    is24x7: 'false'
  });
  ...
  const handleToggleChange = (fieldName, currentValue) => {
    const newValue = currentValue === 'true' ? 'false' : 'true';
    setTextData({ ...textData, [fieldName]: newValue });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!textData.documentState.trim() || !textData.issuingAuthority.trim() || !textData.gstNumber.trim()) {
      return triggerNotification("Please fill in all required text parameters.", "warning");
    }
    ...
    try {
      ...
      // Append text and boolean parameters
      ...
      // Append arrays of multiple document files
      const multiFileKeys = [
        'pharmacyImages',
        'pharmacyCertificates',
        'pharmacyLicenses',
      ...
      const response = await PharmacyAPI.uploadDocuments(payload);

      if (response && response.success) {
        setShowSuccessModal(true);
      } else {
        triggerNotification(response.message || "Failed to submit pharmacy documents.", "error");
      }
    } catch (err) {
      console.error(err);
      triggerNotification(
        err.response?.data?.message || "An error occurred while uploading pharmacy files.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] py-8 px-4 sm:px-6 lg:px-12 antialiased select-none">
      ...
      <div className="max-w-4xl mx-auto mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            <FileText className="text-[#3d3f96]" /> Pharmacy Documents
          </h1>
          <p className="text-slate-500 font-semibold text-xs mt-1">Complete your profile setup by submitting relevant licensing, credentials, and documents.</p>
        </div>
        <button 
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        {/* Left Side: Parameters, descriptions, and toggles */}
        <div className="md:col-span-5 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
            <h2 className="text-sm font-black text-slate-800 border-b border-slate-50 pb-2">Operational Parameters</h2>

            {/* About text input */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">About Pharmacy</label>
              <textarea
                name="about"
                rows={3}
                value={textData.about}
                onChange={handleTextChange}
                placeholder="Brief description or info about your pharmacy store..."
            ...
            {/* Document State */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Document State <span className="text-red-500">*</span></label>
              <input
                required
                type="text"
                name="documentState"
            ...
            {/* Issuing Authority */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Issuing Authority <span className="text-red-500">*</span></label>
              <input
                required
                type="text"
                ...
                onChange={handleTextChange}
                placeholder="e.g. State Drug Controller"
                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-700 outline-none focus:border-[#3d3f96] transition-all"
              />
            </div>
            ...
            {/* GST Number */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">GST Registration Number <span className="text-red-500">*</span></label>
              <input
                required
                type="text"
                name="gstNumber"
            ...
            {/* Drug License Type Selector */}
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Drug License Type <span className="text-red-500">*</span></label>
              <select
            ...
            <div className="pt-3 space-y-3">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Service Settings</p>
              
              {/* Home Delivery */}
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-2">
                  <Truck size={14} className="text-[#3d3f96]" />
                  <span className="text-xs font-bold text-slate-700">Home Delivery Available</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleToggleChange('isHomeDeliveryAvailable', textData.isHomeDeliveryAvailable)}
                  className={`w-12 h-6 rounded-full p-1 transition-colors duration-200 outline-none ${
                    textData.isHomeDeliveryAvailable === 'true' ? 'bg-[#3d3f96]' : 'bg-slate-200'
                  }`}
                >
                  <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${
                    textData.isHomeDeliveryAvailable === 'true' ? 'translate-x-6' : 'translate-x-0'
                  }`} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-700">Open 24x7</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleToggleChange('is24x7', textData.is24x7)}
                  className={`w-12 h-6 rounded-full p-1 transition-colors duration-200 outline-none ${
                    textData.is24x7 === 'true' ? 'bg-[#3d3f96]' : 'bg-slate-200'
                  }`}
                >
                  <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-200 ${
                    textData.is24x7 === 'true' ? 'translate-x-6' : 'translate-x-0'
                  }`} />
                </button>
              </div>
            </div>
        ...
        {/* Right Side: Profile Image banner and multi files attachments dropzone */}
        <div className="md:col-span-7 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            <h2 className="text-sm font-black text-slate-800 border-b border-slate-50 pb-2">Attached Credentials</h2>

            {/* Single File Upload: Profile Banner Image */}
            <div className="space-y-2">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Profile / Display Image (Banner)</p>
              {!files.profileImage ? (
                <label className="border-2 border-dashed border-slate-200 hover:border-[#3d3f96] rounded-2xl p-4 flex flex-col items-center justify-center gap-1 cursor-pointer transition-all bg-slate-50/50">
                  ...
                  <span className="text-xs font-bold text-slate-600">Choose Display Image</span>
                  <input
                    type="file"
                    accept="image/*"
            ...
            {/* Cascaded document sections mapping */}
            {[
              { label: 'Standard Operating Permits (Licenses)', key: 'pharmacyLicenses' },
              { label: 'GSTIN Documentation Form (GST Certificates)', key: 'gstCertificates' },
              { label: 'Authorized Drug License Documents', key: 'drugLicenses' },
              { label: 'Storefront & Premises Images', key: 'pharmacyImages' },
              { label: 'Certification Documentation Copies', key: 'pharmacyCertificates' },
              { label: 'Additional Registration Paperwork', key: 'otherCertificates' }
            ].map((field) => (
              <div key={field.key} className="space-y-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{field.label}</p>
            ...
            >
              {loading ? (
                ...
                <>
                  <span>Submit Pharmacy Records</span>
                  <Upload size={18} />
                </>
              )}
            </button>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] max-w-md w-full p-8 shadow-2xl relative z-10 border border-slate-100 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Records Submitted</h3>
            <p className="text-xs text-slate-500 font-semibold mt-3 leading-relaxed">
              Your credentials and operating permits have been received successfully. Your pharmacy listing is currently <strong className="text-[#3d3f96]">Pending Verification</strong>.
            </p>
            ...
            <div className="mt-4 p-4 bg-amber-50/60 rounded-2xl border border-amber-100/50 flex items-start gap-2.5 text-left">
              ...
              <div>
                <p className="text-[10px] font-black text-amber-800 uppercase tracking-wider">Awaiting Verification</p>
                <p className="text-[11px] text-amber-700 font-medium mt-0.5">Our verification committee will evaluate your registration details and licenses. This generally takes between 24 and 48 hours.</p>
              </div>
            </div>
