...
import CustomizedTiffinDetailModal from './CustomizedTiffinDetailModal';

export default function CustomizedTiffinOrders() {
    // --- Data & Loading States ---
    ...
    const fetchCustomRequests = async () => {
        ...
        try {
            ...
            const response = await FoodAPI.getVendorCustomRequests(params);
            if (response && response.success) {
                setRequests(response.data || []);
            }
        } catch (err) {
            console.error("Error retrieving custom tiffin requests:", err);
            toast.error("Failed to load custom tiffin requests.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const renderStatusBadge = (status) => {
        ...
        if (uppercase === 'CANCELLED' || uppercase === 'REJECTED') {
            return (
    ...
    return (
        <div className="space-y-6 text-left select-none antialiased">
            ...
            {/* --- CUSTOM REQUESTS TABLE VIEW --- */}
            {loading ? (
                <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    <Loader2 className="animate-spin text-[#3d3f96] mb-3" size={36} />
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Loading custom plate requests...</p>
                </div>
            ) : requests.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-20 text-center bg-white rounded-3xl border border-slate-100 shadow-sm border-dashed">
                    ...
                    <h3 className="text-base font-bold text-slate-700">No Custom Requests Found</h3>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm">
                        No personalized custom plate submissions match your filter criteria.
                    </p>
                </div>
            ) : (
                        ...
                        <table className="w-full text-left text-xs border-collapse">
                            <thead>
                                <tr className="border-b border-slate-100 text-slate-400 uppercase font-extrabold bg-slate-50/70 tracking-wider">
                                    ...
                                    <th className="py-4 px-5">Package Specs</th>
                                    <th className="py-4 px-5">Customer &amp; Phone</th>
                                    <th className="py-4 px-5">Delivery Duration</th>
                                    <th className="py-4 px-5">Total Revenue</th>
                                    <th className="py-4 px-5">Status</th>
                                    <th className="py-4 px-5 text-right">Inspect &amp; Action</th>
                                </tr>
                            </thead>
                                        ...
                                        <tr key={req._id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="py-4 px-5">
                                                <div className="space-y-0.5">
                                                    <strong className="text-slate-900 font-black tracking-tight block">{req.bookingId}</strong>
                                                    <span className="text-[10px] text-slate-400 font-bold uppercase">{req.planType}</span>
                                                </div>
                                            </td>

                                            {/* Package Duration & Diet */}
                                            <td className="py-4 px-5">
                                                <div className="space-y-0.5">
                                                    <strong className="text-slate-800 font-extrabold block">{req.packageDays} Days Package</strong>
                                            ...
                                            <td className="py-4 px-5">
                                                <div className="space-y-0.5">
                                                    <strong className="text-slate-800 font-bold block">{req.customerName || "Customer"}</strong>
                                                    <a href={`tel:${req.customerPhone}`} className="text-[11px] font-bold text-[#3d3f96] hover:underline flex items-center gap-1">
                                            ...
                                            <td className="py-4 px-5">
                                                <div className="space-y-0.5 text-[11px] text-slate-600">
                                                    <p className="flex items-center gap-1 font-bold">
                                                        <Calendar size={11} className="text-slate-400" /> {req.startDate}
                                                    </p>
                                                    <p className="text-[10px] text-slate-400 font-semibold pl-4">to {req.endDate}</p>
                                                </div>
                                            </td>

                                            {/* Total Amount */}
                                            <td className="py-4 px-5">
                                                <strong className="font-mono text-sm font-black text-slate-900">₹{req.totalAmount}</strong>
                                            </td>
                                            ...
                                            <td className="py-4 px-5 text-right">
                                                <button
                                                    type="button"
                                                    onClick={() => setInspectRequestId(req._id || req.bookingId)}
            ...
            {/* --- STANDALONE DETAIL & ACTION MODAL --- */}
            <CustomizedTiffinDetailModal 
                requestId={inspectRequestId}
                isOpen={!!inspectRequestId}
                onClose={() => setInspectRequestId(null)}
                onActionComplete={fetchCustomRequests}
            />
