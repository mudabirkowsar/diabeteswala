...
const DoctorRegister = () => {
    ...
    const [states, setStates] = useState(State.getStatesOfCountry('IN')); // Default to India states
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.password.length < 6) {
            return triggerNotification("Password must be at least 6 characters", "warning");
        }
        ...
        try {
            setLoading(true);
            const response = await IndependentDoctorAPI.registerIndependentDoctor(payload);

            if (response.success) {
                triggerNotification(response.message || "Registration successful! Please upload your documents.", "success");
                
                // Store the auth token for subsequent KYC document uploads
                if (response.token) {
                    localStorage.setItem('independentDoctorToken', response.token);
                }

                // Redirect directly to the document upload page for Independent Doctors
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 antialiased">
                ...
                {/* Doctor Name & Specialty/Role Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {/* Role Display */}
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Practitioner Role</label>
                        <div className="relative">
                            <Stethoscope className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
                            <input
                                disabled
                                type="text"
                                value="Independent Medical Practitioner"
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                onChange={handleChange}
                                className="w-20 px-2 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-center text-slate-700 outline-none focus:border-[#3d3f96]"
                                placeholder="+91"
                            />
