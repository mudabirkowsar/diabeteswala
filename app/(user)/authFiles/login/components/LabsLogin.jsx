...
const LabsLogin = () => {
  ...
  const handleLogoutAndClear = () => {
    // Clear stored token and reset login inputs
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Structure payload matching documentation specifications
      const credentials = { 
        email, 
        password,
        category: "Lab" // Explicitly pass verified Lab category parameter
      };

      const response = await LabAPI.loginLab(credentials);

      if (response.success) {
        // Fallback supports both standard profileStatus and potential Accountverify properties
        const verifyStatus = response.profileStatus || response.Accountverify;

        // 1. Handle Approved Account (Full Access)
        if (verifyStatus === 'Approved') {
          if (response.token) {
            localStorage.setItem('labToken', response.token);
          }
        ...
        <div className="space-y-2">
          ...
          <div className="relative group">
            ...
            <input
              ...
              placeholder="apollo.lab@example.com"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative animate-fade-in">
            ...
            <p className="text-xs text-gray-400 mt-3 leading-relaxed">
              Your lab partner application is currently undergoing verification by our administrators. This process typically takes 24-48 hours.
            </p>
      ...
      {statusModal === 'Incomplete' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative animate-fade-in">
            ...
            <p className="text-xs text-gray-400 mt-3 leading-relaxed">
              Your account exists, but valid diagnostic licensing and certification files are required before listing your practice.
            </p>
            <div className="bg-blue-50/50 border border-blue-100/50 rounded-2xl p-4 my-5 text-blue-800 text-xs font-semibold">
              Complete documentation setup to list your lab services on our network.
            </div>
      ...
      {statusModal === 'Rejected' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative animate-fade-in">
            ...
            <p className="text-xs text-gray-400 mt-3 leading-relaxed">
              The administration system was unable to verify your clinical diagnostic credentials.
            </p>
