...
import PharmacyAPI from '../../../../services/PharmacyVendorAPI'; // Service file PharmacyVendorAPI.js
...
const PharmacyLogin = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            // 1. Sanitize payload values to prevent white space issues
            // 2. Explicitly send the category parameter as verified in Postman
            const credentials = {
                email: email.trim(),
                password: password,
                category: "Pharmacy"
            ...
            const response = await PharmacyAPI.loginPharmacy(credentials);

            if (response.success) {
                const verifyStatus = response.profileStatus || response.Accountverify;

                // Approved Account (Full Access)
                if (verifyStatus === 'Approved') {
                    if (response.token) {
                        localStorage.setItem('pharmacyToken', response.token);
                    }
                ...
                // Store intermediate authorization token to grant access to document pages
                if (response.token) {
                    localStorage.setItem('pharmacyToken', response.token);
                }
                ...
                // Rejected Account
                    ...
                    setRejectReason(response.rejectionReason || response.rejectReason || "No specific reason provided.");
        ...
        <>
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            placeholder="wellness.pharmacy@example.com"
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:border-[#3d3f96] focus:ring-4 focus:ring-blue-55 transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            placeholder="••••••••"
                            className="w-full pl-12 pr-12 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:border-[#3d3f96] focus:ring-4 focus:ring-blue-55 transition-all"
                        />
                        <button
                            type="button"
            ...
            {statusModal === 'Pending' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative">
                        ...
                        <p className="text-xs text-gray-400 mt-3 leading-relaxed">
                            Your pharmacy application is currently undergoing verification by our administrators. This process typically takes 24-48 hours.
                        </p>
            ...
            {statusModal === 'Incomplete' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative">
                        ...
                        <p className="text-xs text-gray-400 mt-3 leading-relaxed">
                            Your account exists, but valid pharmacy operating and drug licenses are required before listing your store.
                        </p>
                        <div className="bg-blue-50/50 border border-blue-100/50 rounded-2xl p-4 my-5 text-blue-800 text-xs font-semibold">
                            Complete documentation setup to list your pharmacy on our network.
                        </div>
            ...
            {statusModal === 'Rejected' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2rem] max-w-md w-full overflow-hidden shadow-2xl border border-gray-100 p-8 text-center relative">
                        ...
                        <p className="text-xs text-gray-400 mt-3 leading-relaxed">
                            The administration system was unable to verify your pharmacy operational credentials.
                        </p>
