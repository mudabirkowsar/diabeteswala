...
export default function CustomTiffinStory() {
  ...
  // Absolute precise click-to-scroll handler
  const scrollToMilestone = (index) => {
    if (!containerRef.current) return;
    const targets = [0.18, 0.39, 0.61, 0.83];
    const container = containerRef.current;
    const rect = container.getBoundingClientRect();
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    const containerTop = rect.top + scrollTop;
    const totalScrollableDistance = container.offsetHeight - window.innerHeight;
    const targetScroll = containerTop + targets[index] * totalScrollableDistance;
  ...
  return (
    ...
    >
      <div style={{ height: "380vh" }}>
        ...
        {/* Sticky Stage Viewport (Strictly 90vh with safe bottom padding) */}
        <div className="sticky top-[10vh] h-[90vh] w-full flex flex-col justify-between overflow-hidden px-4 sm:px-6 lg:px-8 pt-3 pb-5 select-none">
          ...
          {/* TOP HEADER */}
          <div className="text-center max-w-2xl mx-auto space-y-1 z-20 shrink-0">
            <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-white/90 backdrop-blur-md text-[#E07A5F] text-[11px] font-black uppercase tracking-widest border border-[#E07A5F]/20 shadow-sm">
              <ChefHat size={13} className="text-[#E07A5F] animate-bounce" />
          ...
          {/* MIDDLE STAGE: Left Laser HUD + Center Vector Tiffin + Right Laser HUD */}
          <div className="relative flex-1 min-h-0 w-full max-w-6xl mx-auto flex items-center justify-center my-auto py-1">
            
            {/* LEFT SIDE CARDS */}
            <div className="hidden lg:block relative h-[360px] w-80 z-20">
            ...
            <div className="relative flex flex-col items-center justify-center mx-auto px-4 z-10">
              ...
              <div className="relative w-[250px] sm:w-[290px] lg:w-[320px] h-[320px] sm:h-[370px] lg:h-[400px] drop-shadow-[0_20px_40px_rgba(224,122,95,0.18)]">
                ...
                >
                  ...
                  {/* 1. MASTER BACK STAND / CARRIER FRAME */}
                  <g opacity="0.4">
                  ...
                  {/* 5. TOP DOME LID & CARRYING HANDLE */}
              ...
              <div className="w-52 sm:w-64 h-3.5 bg-slate-900/10 rounded-full blur-lg mt-1" />

              {/* Mobile Active HUD */}
              <div className="lg:hidden mt-2 w-full max-w-xs">
          ...
          {/* BOTTOM TELEMETRY HUD & FULLY WORKING MILESTONE SCRUBBER */}
          <div className="max-w-4xl mx-auto w-full z-20 shrink-0 pt-1 pb-1">
...
// Sub-Component: Desktop Laser Callout Card
function TiffinLaserCard({ step, progress }) {
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 my-auto h-fit p-5 rounded-3xl bg-white/95 backdrop-blur-2xl border-2 border-slate-200/80 shadow-[0_15px_40px_rgba(224,122,95,0.12)]"
    >
      {/* OPTICAL DIRECTIONAL LASER POINTER */}
      {isLeft ? (
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        <span
          className="text-[9px] font-black uppercase tracking-wider text-white px-2.5 py-0.5 rounded-full shadow-sm"
          style={{ backgroundColor: step.accentColor }}
        >
          {step.badge}
        </span>
      ...
      <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold">
        <span className="text-slate-400 uppercase tracking-wider text-[9px] flex items-center gap-1">
          <Zap size={11} style={{ color: step.accentColor }} /> {step.statLabel}
        </span>
        <span className="font-black text-[11px]" style={{ color: step.accentColor }}>
      ...
      {TIFFIN_STEPS.map((step) => {
        ...
        return (
          <motion.div
            key={step.id}
            style={{ opacity, y }}
            className="absolute inset-0 p-3 rounded-2xl bg-white/95 border-2 border-slate-300 shadow-md text-left flex flex-col justify-between"
          >
            <div>
              <span className="text-[8.5px] font-black uppercase" style={{ color: step.accentColor }}>
                {step.badge}
              </span>
              <h4 className="text-[11px] font-extrabold text-slate-900 truncate">
                {step.title}
              </h4>
            </div>
            <div className="flex items-center justify-between text-[10px] font-black pt-1 border-t border-slate-100" style={{ color: step.accentColor }}>
              <span>{step.role}</span>
              <span className="text-[9px] text-slate-400 font-medium">Auto-Sync</span>
            </div>
          </motion.div>
        );
      })}
...
// Sub-Component: Bottom Scrubber Item (Isolated sub-component ensuring proper React Hook lifecycles)
function ScrubberItem({ step, idx, progress, onSelect }) {
  // Dynamically fills based on current scroll position
  ...
  // Dynamic text opacity for highlighting active step
  ...
  const stepName = step.badge.split("• ")[1] || "Step";

  return (
    <button
      type="button"
      onClick={() => onSelect(idx)}
      className="space-y-1.5 text-left group cursor-pointer focus:outline-none p-1.5 rounded-xl hover:bg-black/[0.03] transition-colors"
    >
      <div className="h-2 w-full bg-slate-200/90 rounded-full overflow-hidden p-0.5 border border-slate-300/60 shadow-inner">
        <motion.div
      ...
      <motion.div
        ...
        className="flex items-center justify-between text-[10px] font-extrabold text-slate-700 group-hover:text-slate-900 transition-colors"
      >
        <span className="truncate">0{idx + 1} {stepName}</span>
        <span
          className="text-[9px] font-black opacity-0 group-hover:opacity-100 transition-opacity ml-1"
          style={{ color: step.accentColor }}
        >
          Jump
        </span>
...
// Sub-Component: Bottom Scrubber & Visible Booking Trigger
function BottomTiffinTelemetry({ progress, onSelectMilestone }) {
  const showCta = useTransform(progress, [0.80, 0.92], [0, 1]);
  const ctaY = useTransform(progress, [0.80, 0.92], [8, 0]);
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <motion.div
        ...
        className="w-full max-w-sm pt-0.5"
      >
        <Link
          href="/food/customtiffin"
          className="w-full inline-flex items-center justify-center gap-2.5 py-2.5 px-5 rounded-xl bg-[#E07A5F] hover:bg-[#C9684F] text-white font-black text-xs sm:text-sm shadow-lg shadow-[#E07A5F]/25 transition-all transform hover:-translate-y-0.5 active:translate-y-0 group"
        >
