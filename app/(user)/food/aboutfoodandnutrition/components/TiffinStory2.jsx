...
import { Sparkles, ArrowRight, ShieldCheck, Flame, Layers, Clock, Zap } from "lucide-react";

// Images mapped to /tiffinanimation/ folder
const TIFFIN_LAYERS = [
  { id: "layer-0", src: "/tiffinanimation/tiffin1.png", label: "Empty Ceramic Base" },
  { id: "layer-1", src: "/tiffinanimation/tiffin2.png", label: "Morning Breakfast Tier", threshold: [0.06, 0.17] },
  { id: "layer-2", src: "/tiffinanimation/tiffin3.png", label: "Modular Mid-Ring Docked", threshold: [0.18, 0.30] },
  { id: "layer-3", src: "/tiffinanimation/tiffin4.png", label: "Gourmet Lunch Main", threshold: [0.31, 0.44] },
  { id: "layer-4", src: "/tiffinanimation/tiffin5.png", label: "Superfood Salad Tier", threshold: [0.45, 0.58] },
  { id: "layer-5", src: "/tiffinanimation/tiffin6.png", label: "Thermal Insulated Lid", threshold: [0.59, 0.72] },
  { id: "layer-6", src: "/tiffinanimation/tiffin7.png", label: "Sealed & Locked Hero", threshold: [0.73, 0.86] }
];

// Modern, global ingredient & features telemetry
const CALLOUTS = {
  left: [
    {
      id: "salad",
      title: "SUPERFOOD SALAD",
      description: "Heirloom greens, edamame, pomegranate & walnuts.",
      start: 0.45
    },
    {
      id: "lunch",
      title: "GOURMET MAIN",
      description: "Grilled protein cubes, tricolor quinoa & roasted greens.",
      start: 0.31
    },
    {
      id: "breakfast",
      title: "MORNING FUEL",
      description: "Chia seed pudding, fresh berries & toasted almonds.",
      start: 0.06
    }
  ],
  right: [
    {
      id: "magnetic-lock",
      title: "AIRTIGHT SEAL",
      description: "Magnetic locking band with 100% leakproof silicone rings.",
      start: 0.73
    },
    {
      id: "thermal-core",
      title: "THERMAL RETENTION",
      description: "Triple-layer insulation keeps meals warm or chilled for 6+ hours.",
      start: 0.59
    },
    {
      id: "modular-stack",
      title: "MODULAR STACK",
      description: "Custom configure 1, 2, or 3 bowls tailored to your schedule.",
      start: 0.18
    }
  ]
};

export default function TiffinStory2() {
  const containerRef = useRef(null);

  // Smooth scroll progression across 450vh track
  ...
  // Fades OUT individual callouts near the finale (0.83 -> 0.88)
  ...
  // Fades IN the big "SEALED / TO GO" finale hero (0.88 -> 0.95)
  ...
  // Click-to-jump milestone scrubber
  ...
  return (
    ...
    >
      <div style={{ height: "450vh" }}>
        ...
        {/* Sticky Stage Viewport (Fits 90vh/100vh cleanly without scroll jumps) */}
        <div className="sticky top-0 h-screen w-full flex flex-col justify-between items-center px-4 sm:px-8 lg:px-12 py-3 sm:py-5 select-none overflow-hidden">
          ...
          {/* MAIN STAGE: Left Column | Center Modular Tiffin | Right Column */}
          <div className="relative flex-1 w-full max-w-[1400px] mx-auto flex items-center justify-between my-auto px-2 sm:px-4">
            ...
            <div className="relative flex-1 flex items-center justify-end pr-2 sm:pr-4 lg:pr-8 z-20 min-w-0">
              ...
              {/* 1. Meal Callouts (Active during scroll 0.0 -> 0.85) */}
              <motion.div
              ...
              {/* 2. Finale Headline: "SEALED" (No clipping) */}
              <motion.div
              ...
              >
                ...
                <p className="text-[10px] sm:text-xs lg:text-sm font-black uppercase tracking-widest text-[#E07A5F] mt-2 hidden sm:block">
                  Airtight • Thermal Locked
                </p>
              </motion.div>
            </div>

            {/* ================= CENTER MODULAR TIFFIN IMAGE STACK ================= */}
            <div className="relative flex flex-col items-center justify-center shrink-0 z-10 mx-1 sm:mx-4">
              ...
              <div className="relative w-[280px] sm:w-[340px] md:w-[390px] lg:w-[440px] xl:w-[480px] h-[390px] sm:h-[480px] md:h-[540px] lg:h-[600px] drop-shadow-[0_20px_35px_rgba(0,0,0,0.10)]">
                ...
                {/* 1. Base Layer (Empty Base - Always visible) */}
                <img
                  src={TIFFIN_LAYERS[0].src}
                  alt="Base Modular Container"
                ...
                {/* 2 to 6: Layered Images that crossfade sequentially */}
                {TIFFIN_LAYERS.slice(1).map((layer) => (
                  <LayerImage
                    key={layer.id}
                    layer={layer}
                    progress={smoothProgress}
            ...
            <div className="relative flex-1 flex items-center justify-start pl-2 sm:pl-4 lg:pl-8 z-20 min-w-0">
              ...
              {/* 1. Feature Callouts (Active during scroll 0.0 -> 0.85) */}
              <motion.div
                style={{ opacity: calloutsGlobalOpacity }}
                className="hidden md:flex flex-col justify-between h-[380px] lg:h-[440px] w-48 lg:w-60 text-left pointer-events-none"
              >
              ...
              {/* 2. Finale Headline: "TO GO" (No clipping) */}
              <motion.div
          ...
          <div className="w-full max-w-2xl mx-auto z-20 shrink-0 flex flex-col items-center gap-2 pt-1 pb-1">
            ...
            >
              <p className="text-xs sm:text-sm font-bold text-slate-700 max-w-lg mx-auto leading-relaxed">
                Chef-prepared meals stacked in modern modular thermal containers, vacuum sealed and delivered fresh to your desk or doorstep.
              </p>
              <div className="flex items-center justify-center gap-3 text-[10px] sm:text-xs font-black uppercase tracking-wider text-slate-500">
                <span className="flex items-center gap-1 text-slate-800">
                  <Sparkles size={11} className="text-amber-500" /> 100% Chef-Cooked
                </span>
                <span>•</span>
                <span className="text-slate-800">BPA-Free Ceramic</span>
                <span>•</span>
                <span className="text-slate-800">Zero Single-Use Plastic</span>
              </div>
            </motion.div>
...
// Sub-Component: Layered PNG/png Image
...
// Sub-Component: Desktop Ingredient Callout with Minimalist Connector
function CalloutItem({ item, progress, side }) {
  ...
  return (
    ...
    >
      ...
      <p
        className={`text-[10px] lg:text-[11px] text-slate-500 font-medium tracking-normal mt-0.5 max-w-[180px] lg:max-w-[210px] leading-snug ${
          isLeft ? "pl-0" : "pl-7 lg:pl-10"
        }`}
      >
        {item.description}
      </p>
...
function BottomDashScrubber({ progress, onSelect }) {
  const totalSteps = TIFFIN_LAYERS.length + 1; // 7 layers + 1 finale step
...
function DashItem({ index, totalSteps, progress, onSelect }) {
  ...
  return (
    <button
      ...
      title={index === totalSteps - 1 ? "Sealed to Go" : TIFFIN_LAYERS[index]?.label}
    >
