...
export default function SubscriptionPlans() {
  ...
  const fetchAllPlans = async () => {
    ...
    try {
      ...
      setFeedbackMsg({ type: 'error', text: 'Failed to load subscription tiers.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  // Helper for auto-clearing feedback notifications
  useEffect(() => {
    if (feedbackMsg) {
  ...
  const handleToggleStatus = async (planId) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        setFeedbackMsg({ type: 'success', text: response.message || 'Status updated successfully!' });
      }
    } catch (err) {
      console.error("Error toggling plan status:", err);
      setFeedbackMsg({ type: 'error', text: err.response?.data?.message || 'Failed to toggle status.' });
    } finally {
  ...
  const handleDeletePlan = async (planId) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        setFeedbackMsg({ type: 'success', text: 'Subscription plan removed successfully.' });
      }
    } catch (err) {
      console.error("Error deleting plan:", err);
      setFeedbackMsg({ type: 'error', text: err.response?.data?.message || 'Failed to delete plan.' });
    } finally {
      setActionLoadingId(null);
    }
  };
  ...
  const openEditModal = async (plan) => {
    try {
      // Fetch populated single plan details
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto animate-fade-in py-4 text-left select-none">
      ...
      {feedbackMsg && (
        <div className={`p-4 rounded-2xl border flex items-center gap-2.5 text-xs font-bold transition-all ${feedbackMsg.type === 'success'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={targetId}
                className={`bg-white rounded-3xl border border-slate-200/80 flex flex-col justify-between shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group ${!isActive ? 'opacity-65 saturate-[0.5]' : ''
                  }`}
              >
                <div>
                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        {["Breakfast", "Lunch", "Dinner"].map(meal => {
                          ...
                          return (
                            <span
                              key={meal}
                              className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-colors ${isAllowed
                ...
                <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex justify-between items-center">
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                    className={`flex items-center gap-1.5 text-[10px] font-bold border px-3 py-1.5 rounded-full transition-all duration-300 cursor-pointer disabled:opacity-50 ${isActive
      ...
      <CreateTiffin
        ...
        onSuccess={() => {
          fetchAllPlans(); // Refetch plans after successful creation / update
          setFeedbackMsg({
            type: 'success',
            text: modalMode === 'create' ? 'Subscription tier created!' : 'Plan updated successfully!'
          });
        }}
