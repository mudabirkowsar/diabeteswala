...
import { 
    FaEye, FaColumns, FaCheck, FaTimes, FaPills, 
    FaPrescriptionBottle, FaIndustry, FaInbox 
} from "react-icons/fa";

export default function PendingMedicines() {
    // Initial Seed Data with realistic medicine names and manufacturers
    const [medicines, setMedicines] = useState([
        { 
            id: 1, 
            name: "Metformin ER 500mg", // Standard Diabetes medicine
            manufacturer: "Sun Pharmaceutical Industries", 
            mrp: 150, 
            bestPrice: 120, 
            prescriptionRequired: true, 
            image: "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=100&auto=format&fit=crop" 
        },
        { 
            id: 2, 
            name: "Atorvastatin 10mg", // Cholestrol medicine
        ...
        { 
            id: 3, 
            name: "Paracetamol 650mg (Dolo-650)", // Standard pain relief / fever medicine
    ...
    const handleApprove = (id) => {
        // Remove item dynamically on approval
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto px-4 py-8 select-none animate-fadeIn">
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.015)]">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                            type="checkbox" 
                            checked={visibleCols.medicineName} 
                            onChange={() => toggleColumn("medicineName")}
                        ...
                        />
                        <span className="text-sm font-semibold text-gray-600 group-hover:text-gray-900 transition-colors">Medicine Name</span>
                    </label>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                            type="checkbox" 
                            checked={visibleCols.prescription} 
                            onChange={() => toggleColumn("prescription")}
                            className="w-4.5 h-4.5 text-[#3D3F96] bg-gray-50 border-gray-200 rounded-lg focus:ring-1 focus:ring-[#3D3F96]/30 cursor-pointer accent-[#3D3F96]"
                        />
                        <span className="text-sm font-semibold text-gray-600 group-hover:text-gray-900 transition-colors">Prescription Required</span>
                    </label>
            ...
            {/* 2. PENDING MEDICINES DATA TABLE */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.015)] overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-[#3D3F96]/5">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#3D3F96]/15 text-[#3D3F96] flex items-center justify-center">
                            <FaPrescriptionBottle className="text-lg animate-pulse" />
                        </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm min-w-[850px] table-auto">
                        <thead>
                            <tr className="text-[11px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50/50 border-b border-gray-100">
                                ...
                                {visibleCols.medicineName && <th className="text-left px-6 py-4">Medicine Name</th>}
                                ...
                                {visibleCols.prescription && <th className="text-center px-6 py-4">Prescription Required</th>}
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {medicines.map((item, index) => (
                                ...
                                >
                                    ...
                                    {/* Prescription Badge */}
                                    {visibleCols.prescription && (
                                        <td className="px-6 py-4 text-center">
                                            {item.prescriptionRequired ? (
                                                <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-rose-50 text-rose-600 border border-rose-100 uppercase tracking-wide">
                                                    Required
                                                </span>
                                            ) : (
                                                <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100 uppercase tracking-wide">
                                                    Not Required
                                                </span>
                                            )}
                                        </td>
                            ...
                            {/* Empty State when no pending medicines remain */}
                            {medicines.length === 0 && (
                                <tr>
                                    <td colSpan={8} className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center justify-center gap-3">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-bold text-slate-700">No Pending Medicines</h4>
                                                <p className="text-xs text-slate-400 mt-1">All requested medicines have been processed successfully.</p>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
